...
Aclient::Aclient()
{
    ...
    oscReciever = new OSCReceiver();
    oscSender = new OSCSender();
    
    oscReciever->addListener (this);
    if (oscReciever->connect (8001) == false)
        std::cout << "Aclient: unable to connect on port 8001\n";
    
    if (oscSender->connect ("127.0.0.1", 8000) == false)
        std::cout << "Aclient: unable to connect on port 8000\n";
}
...
Aclient::~Aclient()
{
    ...
    oscReciever->disconnect();
    oscReciever->removeListener (this);
...
void Aclient::oscMessageSend (const OSCMessage& message)
{
    oscSender->send (message);
}

void Aclient::oscMessageReceived (const OSCMessage& message)
{
...
class AserveConnection :	public InterprocessConnection
{
	...
	void retrieveNoteMessage(MidiMsg& midiMsg) 
	{
		...
		{
			midiMsg = threadData.note.removeAndReturn (0);
